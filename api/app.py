...
def _get_all_pages(url, access_token, limit=1000):
    """
    Fetches all pages from a Spotify endpoint, with a hard limit to prevent memory issues.
    """
    items = []
    endpoint = url
    while endpoint and len(items) < limit:
        # Adjust limit on the fly for the last page
        ...
        # Stop if there are no more pages or if we have enough items
...
def timeline():
    ...
    try:
        ...
        # --- NEW: Fetch a maximum of 1000 recent saved tracks ---
        saved_tracks_items = _get_all_pages('me/tracks?limit=50', access_token, limit=1000)
        ...
        for key in sorted(phases.keys(), key=get_sort_key):
            ...
            for track in phase_tracks:
                ...
                if track.get('artist_id') in artist_genres_map:
                    for genre in artist_genres_map[track['artist_id']]:
        ...
        return render_template('login.html', error="An error occurred during analysis. Please try logging out and back in again.")
