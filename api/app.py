...
def get_phase_details():
    ...
    try:
        ...
        # Determine a single, representative cover URL for the phase
        album_counts = defaultdict(int)
        for track in tracks_in_phase:
            if track.get('album'):
                album_counts[track['album']['id']] += 1
        
        most_frequent_album_id = max(album_counts, key=album_counts.get) if album_counts else None
        cover_url = 'https://placehold.co/128x128/121212/FFFFFF?text=?'
        if most_frequent_album_id:
            for track in tracks_in_phase:
                if track.get('album') and track['album']['id'] == most_frequent_album_id and track['album'].get('images'):
...
# This is only for local development, Vercel will use the 'app' object directly
